 ...
 _________________________________________________________ -->
    <div id="top">
	  ...
	  if (isset($_SESSION["username"])){
		  echo '<li><a href="doAction.php?act=login">';
 ...
 _________________________________________________________ -->
	  ...
	  oci_execute($stmt);
			 ...
			 while(oci_fetch_array($stmt)){
                 $date[$i]=oci_result($stmt,'DD');
				 $total[$i]=oci_result($stmt,'TOTAL');
				 $id[$i]=oci_result($stmt,'ORDER_ID');
				 $i++;
...
<?php 									
					...
					for($c=0;$c<$i;$c++){					
                            echo '<th>'. $id[$c].'</th><td>'.$date[$c].'</td><td>$'.$total[$c].'</td><td><span class="label label-info">Being prepared</span></td><td><button onclick="view('.$id[$c] .')" class="btn btn-primary btn-sm">View</a></td></tr>';
 ...
 _________________________________________________________ -->
    ...
    <script>
	function view(value){
		var url='customer-order.php?id='+value;
